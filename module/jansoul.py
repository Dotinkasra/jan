...
class Jansoul:
    ...
    def _get_user_with_seat(self, seat: int) -> User:
        """シート番号のUserを取得する
            ...
            if fangchong is None: #ツモ
                ...
                for user in self.users:
                    if not user.seat == hule['seat']:
                        if ippatsu > 0:
                            user.transaction.append(Hule(to=winner.nickname, cnt=ippatsu, bonus=Bonus.ippatsu, zimo=True))            
                        if ura_dora > 0:
                            user.transaction.append(Hule(to=winner.nickname, cnt=ura_dora, bonus=Bonus.ura_dora, zimo=True))
                        if aka_dora > 0:
                            user.transaction.append(Hule(to=winner.nickname, cnt=aka_dora, bonus=Bonus.aka_dora, zimo=True))     
                        if yiman > 0:
                            user.transaction.append(Hule(to=winner.nickname, cnt=yiman, bonus=Bonus.yiman, zimo=True))    
                        if allstar > 0:
                            user.transaction.append(Hule(to=winner.nickname, cnt=allstar, bonus=Bonus.allstar, zimo=True))
                ...
                if ura_dora > 0:
                    fangchong.transaction.append(Hule(to=winner.nickname, cnt=ura_dora, bonus=Bonus.ura_dora, zimo=False))                   
                if aka_dora > 0:
                    fangchong.transaction.append(Hule(to=winner.nickname, cnt=aka_dora, bonus=Bonus.aka_dora, zimo=False))                   
                if yiman > 0:
